$(document).ready(function() {
    ...
    $('.confirm').click(function(){
	   	if(!confirm('Er du sikker?')){
			...
			if(window.location.pathname == "/status") {
				if (!confirm("Dou you want to send mails?")) {
					$("#raport_emails").val('');
				}
			}
		}
	...
	$('.repair_check').click(function(event) {  //on click
		const id = this.id.split("_")[1];
		var href = '';

        if(this.checked) {
			if ($('#print_repairs').attr('href') == "#") {
				href = window.location.origin + "/receipt/print_repairs/" + $('input[name=extra_access_to_order_id]').val() + "?" + '&repairs_list[]=' + id;
				$('#print_repairs').attr('target', '_blank')
			} else {
				href = $('#print_repairs').attr('href') + '&repairs_list[]=' + id;
			}
			$('#print_repairs').attr('href', href);
			var repairs_id = $('#pay_repair_').data('id');
			repairs_id.push(id);
			$('#pay_repair_').data('id', repairs_id);
        }else{
			var href = $('#print_repairs').attr('href').replace('&repairs_list[]='+id, '');
			if (href.includes('&repairs_list[]')) {
				$('#print_repairs').attr('href', href);
			} else {
				$('#print_repairs').attr('href', '#');
				$('#print_repairs').attr('target', '')
			}
			var repairs_id = $('#pay_repair_').data('id');
			if (array.indexOf(id) > -1) {
				repairs_id.splice(index, 1);
			}
			$('#pay_repair_').data('id', repairs_id);
        }
    });
...
function payment_type(){
	$('select[name=payment_type]').change(function(){
	   ...
	   if(value == 'cash'){
		   // show checkboks
		   $('.hidden_checkboks').show();
	   }else{
		   $('.hidden_checkboks').hide();
		   $('.hidden_checkboks input').val(0);
	   }
