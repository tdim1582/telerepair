...
$Garanti	=	$this->global_model->selectAllData('garanti', array('id' => $phone[0]->garanti), '', '', '', '', 'text');
?>
...
<body>
	...
	<div style="width: 220px; padding: 0px;">
		...
		if ($type == 'access') {
			...
			$total = $phone[0]->price;
			$original_total = $phone[0]->price;
			?>

			<h1 style="font-size: 17px; margin-top: 70px; margin-bottom: 20px">
				<center>Tilbehørs kvittering</center>
			</h1>
			...
			<table style="width:220px;">
                            ...
                            <?php foreach ($order_item->result() as $item) {?>
                            <tr>
                                <td style="font-size: 12px" ><?php echo $item->product . ' - ' . $item->part; ?></td>
                                <td style="font-size: 12px" ><?php echo $item->qty; ?></td>                        
                                <td colspan="2" style="font-size: 12px;text-align:right"><?= number_format($item->price, 0, ',', '.'); ?> kr</td>
                            </tr>
                            <?php  } ?>
                            ...
                            ?>
                            <tr>
                                <td colspan="3" style="font-size: 12px;  padding: 10px 0px;">
                                    <div style="border-bottom: 1px dashed #2e2e2e;"></div>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td style="font-size: 12px" ><?= $extra->part; ?></td>
                            ...
                            ?>
                            <tr>
                                <td colspan="4" style="font-size: 12px;  padding: 10px 0px;">
                                    <div style="border-bottom: 1px solid #2e2e2e;"></div>
                                </td>
                            </tr>

                            <tr>
                                <td colspan="2" style="font-size: 12px" >Subtotal</td>
                                <td colspan="2" style="font-size: 12px; text-align: right;" ><?= number_format($phone[0]->subtotal, 0, ',', '.'); ?> kr</td>
                            </tr>
                            <?php if ($phone[0]->discount || $original_total < $phone[0]->subtotal) { ?>
                            <tr>
                                <td style="font-size: 12px" >Rabat</td>
                                <!-- <td colspan = "3" style="font-size: 12px; text-align: right;">-<?= number_format(($phone[0]->discount * $phone[0]->subtotal) / 100, 0, ',', '.'); ?> kr</td> -->
                                <td colspan = "3" style="font-size: 12px; text-align: right;">-<?= number_format($phone[0]->subtotal - $original_total, 0, ',', '.'); ?> kr</td>
                            </tr>
                            <?php } ?>
                            
                            <tr style="font-weight: bold">
                                <td style="font-size: 12px; padding-top:10px;" ><b>Total</b></td>
                                <td colspan = "3" style="font-size: 12px; text-align: right; padding-top:10px;" ><?= number_format($total, 0, ',', '.'); ?> kr</td>
                            </tr>
                            ...
                            <tr>
                                <td style="font-size: 12px; padding-top:10px;" colspan="3" width="100px"><b><?php echo $Garanti[0]['text']; ?></b></td>
                            </tr>
                        </table>
		<?php
		} elseif ($type == 'sale') {
			...
			<h1 style="font-size: 17px; margin-top: 70px; margin-bottom: 30px">
				<center>Salgskvittering</center>
			</h1>
			...
			<table style="width:220px">
				...
				<tr>
					...
					<td style="font-size: 12px"><?php if ($phone[0]->insurance) : echo 'Ja (+' . number_format($phone[0]->insurance_price, 2, ',', '.') . ' kr, ' . $phone[0]->insurance_years . ' år)';
													else : echo 'Nej';
													endif; ?></td>
				</tr>
	...
	<?php var_dump('!@#$%^&*(^%$#@!$%^&$#@!$%^&*%$#@!$%^&*^%$#@!4');?>
		...
		<?php } ?>
		<div style="margin-top: 15px; font-size: 13px;">På gensyn</div>
