...
class Global_model extends CI_Model {
    ...
    function get_user_name_by_id($id){
        ...
        $result = $this->db->get('users_kasse')->result();
    ...
    function get_sales_all_users(){
        $this->db->order_by('id','desc');
        $this->db->where('active',1);
        
        $query = $this->db->get('users_kasse');
    ...
    //Den henter data fra tabellen "orders", som registrerer bruger ID "uid". Derefter kan de enkelte bruger ID'er kobles sammen med brugernavne fra tabellen "users_kasse" feltet "name".
    
    function calculate_sale_by_user($user_id = false, $month = false,$boutique_id = false){
        ...
        #Calculate from and to times from the month.
        $from = strtotime($month.' 00:00:00');
        $to   = strtotime($month.' 23:59:59');
        ...
        #Select store based on the function or choose active store (boutique).
        if($boutique_id){
            if($boutique_id != 'all'){
                $this->db->where('boutique_id',$boutique_id);
            }
