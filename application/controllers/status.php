...
class Status extends CI_Controller {
	...
	public function index($id = false)
	{
		...
		if($data['counting_from_yesterday']){
			$data['id'] = $data['counting_from_yesterday'][0]->id;
			...
			$date = date("Y-m-d",$data['counting_from_yesterday'][0]->created_timestamp);
			$timeonday = date("H:i",$data['counting_from_yesterday'][0]->created_timestamp);
		...
		$data['card'] = $this->global_model->calculate_sale_by_payment_type('card',$date,false,$timeonday);
		$data['cash'] = $this->global_model->calculate_sale_by_payment_type('cash',$date,false,$timeonday);
		$data['mobilepay'] = $this->global_model->calculate_sale_by_payment_type('mobilepay',$date,false,$timeonday);
		$data['invoice'] = $this->global_model->calculate_sale_by_payment_type('invoice',$date,false,$timeonday);
		$data['webshop'] = $this->global_model->calculate_sale_by_payment_type('webshop',$date,false,$timeonday);
		$data['loan'] = $this->global_model->calculate_sale_by_payment_type('loan',$date,false,$timeonday);
		$data['nettalk'] = $this->global_model->calculate_sale_by_payment_type('nettalk',$date,false,$timeonday);
	...
	function generate($id = false){
		...
		if($data['counting'][0]->boutique_id != 45454545454545){
			...
			$data['cashPrevDay'] = $this->global_model->calculate_sale_by_payment_type('cash',date("Y-m-d",$prevdayTimestamp));
